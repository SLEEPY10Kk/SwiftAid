...
import com.example.swiftaid.emergency.EmergencyCommunicationLevel
import com.example.swiftaid.emergency.EmergencyConnectionCoordinator
import com.example.swiftaid.emergency.EmergencyConnectionResult
...
class SOSOverlayActivity : AppCompatActivity() {
    ...
    private fun executeSos() {
        ...
        LocationFallbackCascade(this).fetch { locationResult ->
            EmergencyConnectionCoordinator.dispatch(
                context = this,
                locationResult = locationResult,
                sosType = if (manualSos) "MANUAL" else "CRASH"
            ) { result ->
                runOnUiThread {
                    statusText.text = statusFor(result)
    ...
    private fun statusFor(result: EmergencyConnectionResult): String {
        return when (result.level) {
            EmergencyCommunicationLevel.ONLINE_CLOUD -> getString(R.string.sos_status_cloud_sent)
            EmergencyCommunicationLevel.SMS_RELAY -> resources.getQuantityString(
                R.plurals.sos_status_sms_relay,
                result.smsResult.attemptedContacts,
                result.smsResult.attemptedContacts
            )
            EmergencyCommunicationLevel.PENDING_SYNC -> getString(R.string.sos_status_pending_sync)
        }
    }
